...
function computeBillStatus(
  ...
  const others = participants.filter((p) => toIdString(p.userId) !== createdById);
...
export async function PATCH(req: NextRequest, context: { params: { billId: string } }) {
  try {
    ...
    const ownerId = toIdString(billDoc.createdBy);
    if (!ownerId) {
    ...
    if (body.slipInfo) {
      participants[idx].slipInfo = {
        ...
        checkedAt: body.slipInfo.checkedAt
          ? new Date(body.slipInfo.checkedAt)
          : participants[idx].slipInfo?.checkedAt,
        verified:
          typeof body.slipInfo.verified === 'boolean'
            ? body.slipInfo.verified
            : participants[idx].slipInfo?.verified ?? false,
      };
    }
    ...
    if (action) {
      try {
        ...
        console.error('⚠️ notifyBillStatusChanged failed:', err);
      }
    }
