...
function CreateBillPageInner() {
  ...
  function getMeByEmail(users: User[], email?: string | null): User | undefined {
  ...
  useEffect(() => {
    async function fetchUsers() {
      const token = localStorage.getItem('token'); // ถ้าคุณยังใช้ token อยู่
  ...
  const resetForm = () => {
    ...
    setParticipants(() => {
      const me = getMeByEmail(users, currentUserEmail ?? null);
      return me ? [{ userId: me._id, name: me.name, amount: 0 }] : [{ userId: '', name: '', amount: 0 }];
    });
  ...
  const handleAddItems = () => {
    setItemList([...itemList, { items: '', price: '' }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    const normalizedTotalPrice = cleanedItems.reduce(
    ...
    const res = await fetch('/api/bills', {
      ...
      body: JSON.stringify({
        ...
        splitType,
        participants: cleanedParticipants, // ✅ คงชื่อเดิมของคุณ
        description,
        items: cleanedItems,
      }),
    });
    ...
    if (res.ok) {
      ...
      resetForm(); // ✅ ล้างฟอร์มหลัง save สำเร็จ
      ...
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl p-8 relative">
        ...
        <div className="border-dashed border-2 border-gray-300 rounded-xl p-8 text-center mb-6">
          ...
          <p className="text-xs text-gray-400 mt-3">
            * OCR ปิดชั่วคราว (ยังไม่ประมวลผลรูป)
          </p>
        </div>
        ...
        <div className="mb-6 text-center">
          ...
          <p className="text-sm text-[#4a4a4a]">
            Based on {participants.length} participants
          </p>
        </div>
